...
int main ( int argc, char * argv[] ) {
  ...
  int N = 108, c;
  double L = 0.0;
  double rho = 0.6, T = 2.0, P = 1.0, beta = 1.0;
  double rc = 3.5, rc2 = rc*rc, vir, p_sum, rho_sum, pcor, V;
  double E, esum, rr3, ecor, ecut;
  double dr = 0.2, dlnV = 0.25;
  int i;
  int nCycles = 10000, nSamp, nEq = 1000;
  ...
  for (i=1;i<argc;i++) {
    ...
    else if (!strcmp(argv[i],"-rho0")) rho=atof(argv[++i]);
  ...
  /* Compute the initial side-length */
